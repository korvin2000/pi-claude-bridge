...
**A file snapshot is carried only when pi recorded the tool call it hung off.** Claude Code attaches a file's fresh contents as an `edited_text_file` when something changes it underneath a turn — overwhelmingly off a tool result rather than a prompt (over 290 real records: 268 off `Bash`, 21 off a prompt, 1 off `Write`). These are now carried, anchored on the answered call's `tool_use_id`, which survives a rebuild because Claude Code's `tool_use` id *is* the id pi records as its `toolCallId`. The case that still drops is an AskClaude child running a tool of Claude Code's own inside the shared session: pi never recorded that call, so the rebuild is not reproducing that turn either and the attachment has nothing to hang on. `@file` expansions are carried as before.
