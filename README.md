...
**A file snapshot is carried only when pi recorded the tool call it hung off.** Claude Code attaches a file's fresh contents as an `edited_text_file` when something changes it underneath a turn — this is its staleness tracking re-reading a file it already knows about, not a log of its own edits. Measured over 290 real records on one machine: 268 hang off a `Bash` result, 21 off a prompt, 1 off `Write`. Both shapes are now carried — the prompt-parented ones by the existing ordinal, the tool-result ones by the answered call's `tool_use_id`, which survives a rebuild because Claude Code's `tool_use` id *is* the id pi records as its `toolCallId`.

Two notes on how often that second route actually fires here. The provider path runs Claude Code with `tools: []`, so it does not author these from its own tool use; the reachable provider-path case is the prompt-parented one, where a file the user `@`-mentioned changed on disk between turns. The tool-result route does fire for an AskClaude child running Claude Code's own tools inside the shared session — but pi never recorded those calls, so the id matches nothing in the rebuilt transcript and the attachment is dropped, which is the right answer for context whose parent turn is not being rebuilt either. `@file` expansions are carried as before.
