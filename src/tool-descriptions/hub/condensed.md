...
Jobs auto-deliver on finish. NEVER poll. A settled row seen by `jobs`/`wait` IS the delivery, suppressing the duplicate.

- **The user is NOT a peer.** `Main` answers them only in plain text; a `send` shows them a tool card.
- `send` (`to`) — fire-and-forget, NEVER blocks; revives a parked peer. Address peers by exact id from `list`, NEVER an invented one. `failed` = peer gone, do not retry. Lead with the answer, NEVER quote, set `replyTo`. Prose only; pass payloads as `local://` URLs.
...
- `inbox` drains queued messages. `cancel` kills hung jobs by `ids`. `jobs` snapshots without waiting, naming subagents with no job row.
...
- `start` runs `application` + `args`. Readiness MUST be observed, never assumed from process creation: `ready.log` and/or `ready.port`, both passing if both given. Pattern fields are `u`-flag JavaScript `RegExp`s — inline `(?i)` is REJECTED, write `[Rr]eady`.
